...
int main() {
	...
	//Projection Matrix - 45 degrees FoV, aspect ratio, near far range (0.1 - 100 units)
	int width, height;
	...
	glm::mat4 View = glm::lookAt(
		...
		glm::vec3(0, 0, 0),	// Camera looks at this point (origin)
		glm::vec3(0, 1, 0)	// Up vector
	);

	//Model Matrix
	glm::mat4 Model = glm::mat4(1.f); // reseting the matrix
	...
	// Handle for our mvp matrix (uniform variable (doesn't change for the whole draw call))
	...
	//Face culling (only visible on the normal side)
	...
	while (!glfwWindowShouldClose(window)) {
		...
		keyboard();
		Model = glm::mat4(1.f);
		Model = glm::translate(Model, glm::vec3(movX, 0.5f + movY, 0.f));
