...
namespace nou
{
	...
	void ParticleUtility::VerticalConeEmit(float tanTheta, float speed, glm::vec3& pos, glm::vec3& vel)
	{
		...
		vel = speed * glm::normalize(
			glm::vec3(tanTheta * x, 1.f, tanTheta * z)) * rotMat;
	}

	void ParticleUtility::SetRot(float angle)
	{
		if (angle == 0.f) {
			rotMat = glm::mat3(1.f);
		}
			...
			rotMat = glm::mat3(
				glm::cos(angle), -glm::sin(angle), 0.f,
	...
	void CParticleSystem::Update(float deltaTime)
	{
		...
		float emissionTime = 1 / m_data->param.emissionRate;

		if (m_data->param.startAngle != m_data->param.endAngle)
			m_data->param.currentAngle = glm::mix(
				m_data->param.startAngle, m_data->param.endAngle, float(m_data->counter) / m_data->count);
		ParticleUtility::SetRot(m_data->param.currentAngle);
		...
		for (int index = static_cast<int>(m_data->lastAlive); index >= 0; --index)
		{
			...
			if (!m_data->alive[i])	continue;

			m_data->lifetime[i] -= deltaTime;

			if (m_data->lifetime[i] <= 0.f) {
			...
			float lifetimeT = 1.f - (m_data->lifetime[i] / m_data->param.lifetime);

			m_data->color[i] = glm::mix(m_data->param.startColour, m_data->param.endColour, lifetimeT);
			m_data->size[i] = glm::mix(m_data->param.startSize, m_data->param.endSize, lifetimeT);

			m_data->pos[i] += (deltaTime * m_data->velocity[i] -
				glm::vec3(0.f, 0.5f, 0.f) * m_data->param.gravity * deltaTime * deltaTime) *
				glm::mix(m_data->param.startSpeed, m_data->param.endSpeed, lifetimeT);

			m_data->velocity[i] -= m_data->param.gravity * glm::vec3(0.f, deltaTime, 0.f);

			m_data->viewPos[i] = modelview * glm::vec4(m_data->pos[i], 1.f);
	...
	void CParticleSystem::Emit()
	{
		...
		if (m_data->numAlive < m_data->count)
		{
			...
			m_data->color[i] = m_data->param.startColour;
			...
			++(m_data->counter);
			if (m_data->counter >= m_data->count) {
				m_data->counter = 0;
			}
		}
	}
