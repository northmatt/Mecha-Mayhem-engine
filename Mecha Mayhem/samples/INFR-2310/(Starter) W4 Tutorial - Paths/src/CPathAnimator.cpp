...
namespace nou
{
	...
	{
		size_t keySize = keypoints.size();

		//TODO: Complete this function.
		if (keySize == 0 || m_segmentTravelTime == 0)
			return;
		...
		while (m_segmentTimer > m_segmentTravelTime) {
			...
			m_segmentIndex += 1;
			if (m_segmentIndex >= keySize)		m_segmentIndex = 0;
		}
		...
		if (m_mode == PathSampler::PathMode::CATMULL) {
			...
			//catmull, path segment is between p1 and p2
			//segment gonna be p1

			p1_ind = m_segmentIndex;
			p0_ind = (p1_ind == 0) ? keySize - 1 : p1_ind - 1;
			p2_ind = (p1_ind + 1) % keySize;
			p3_ind = (p2_ind + 1) % keySize;
		...
		else if (m_mode == PathSampler::PathMode::BEZIER) {
			if (keySize < 6 || keySize % 3 != 0) {
				m_owner->transform.m_pos = keypoints[0]->transform.m_pos;
				return;
			}

			while (m_segmentIndex % 3 != 0)	++m_segmentIndex;
			if (m_segmentIndex >= keySize)	m_segmentIndex = 0;

			size_t p0_ind, p1_ind, p2_ind, p3_ind;
			glm::vec3 p0, p1, p2, p3;

			//Bezier, path segment is between p0 and p3
			//segment gonna be p0
