...
template<typename T>
T Bezier1(const T& p0, const T& p1, const T& p2, float t)
{
...
{
	return LERP(Bezier1(p0, p1, p2, t), Bezier1(p1, p2, p3, t), t);
}
	...
	while (!App::IsClosing() && !Input::GetKey(GLFW_KEY_ESCAPE))
	{
		...
		if (ImGui::Button("Add")) {
			...
			if (points.size() > 1) {
				...
				p->transform.m_pos = lastP->transform.m_pos + glm::vec3(0.2f, 0.f, 0.f);
			}
		}
		...
		ImGui::SameLine(0.f, 10.f);
		if (sampler.m_mode == PathSampler::PathMode::CATMULL) {
